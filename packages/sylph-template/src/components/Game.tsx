...
const Text = (props: TextIntrinsicProps) => (
    <text scale={0.5} style={{fill: "lightblue", fontFamily: "sans-serif", fontSize: 70}} {...props} >
        {props.children}
    </text>
);
...
const MainSprite = (props: MainSpriteProps) => {
    ...
    return (
        <sprite
            texture={texture()}
            tint={props.tint}
            pivot={{x: (texture()?.width||0)/2, y: (texture()?.height || 0)/2}}
            scale={0.3}
            x={props.position.x}
            y={props.position.y}
            filters={[props.blur]}
...
export const Game = () => {
    ...
    const [blurFilter, setBlurFilter] = createSignal(new BlurFilter({strength: 10}), {equals: false});
