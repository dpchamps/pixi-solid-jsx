...
const createWanderingEntity = (
  ...
  const states = {
    toCreate: (): OnNextFrameQuery<{ entityId: string }> => ({
      ...
      tick: ({ entityId }) => {
        entityList.createEntity({
          ...
          x: randomBetween(
            -(width / 2),
            applicationState.application.canvas.width + width / 2,
          ),
          y: randomBetween(
            -(width / 2),
            applicationState.application.canvas.width + width / 2,
    ...
    created: (): OnNextFrameQuery<{
      entity: EntityProps;
      deltaTime: number;
    }> => ({
      query: (applicationState) => ({
        entity: entityList.expectEntity(id),
        deltaTime: untrack(applicationState.time.deltaTime),
      }),
  ...
  return (
    <>
      ...
      <container zIndex={10000}>
        ...
        <text y={25} zIndex={10000}>
          Entity Count: {`${entityList.entities().length}`}
        </text>
      </container>
    </>
  );
