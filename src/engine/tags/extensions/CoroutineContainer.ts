...
export const CoroutineContainer = <T extends JSX.Element>(
  ...
  const [coroutine, setCoroutine] =
    createSignal<ReturnType<typeof startCoroutine>>();
  ...
  createSynchronizedEffect(
    () => ({
      ...
      shouldReplay: props.replay,
      stopped: coroutine()?.stopped(),
    }),
    ({ shouldStart, shouldReplay, stopped }) => {
