...
import {
  chainCoroutine,
  createEasingCoroutine, createRepeatableCoroutine,
  startCoroutine, waitMsCoroutine,
...
export const CoroutineContainer = <T extends JSX.Element>(
  ...
  const shouldStart = () => typeof props.shouldStart === "undefined" ? true : props.shouldStart;
  const baseCoroutineConstructor = () => chainCoroutine(
  ...
  const baseCoroutine = props.replay ? createRepeatableCoroutine(baseCoroutineConstructor) : baseCoroutineConstructor();
  const scheduledCoroutine = startCoroutine(baseCoroutine, false);

  createSynchronizedEffect(
      shouldStart,
      (shouldStart) => {
        return shouldStart ? scheduledCoroutine.resume() : scheduledCoroutine.pause()
      }
  );
  ...
  return () => props.children(getNext, scheduledCoroutine.stopped || (() => false));
};
