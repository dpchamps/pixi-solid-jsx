import {createComputed, createSignal, onCleanup} from "solid-custom-renderer/patched-types.ts";
import {Point} from "../libs/Point.ts";

type ButtonType = "Main" | "Auxiliary" | "Secondary" | "Forth" | "Fifth" | "Unknown";
...
export const createMouse = (element: MouseLikeEl) => {
    ...
    const [wheel, setWheel] = createSignal<Pick<WheelLikeEvent, "deltaZ" | "deltaY" | "deltaX">>();
    ...
    const onWheelEvt = (evt: WheelLikeEvent) => setWheel({
        deltaX: evt.deltaX,
        deltaY: evt.deltaY,
        deltaZ: evt.deltaZ,
    });

    const onMousemove = (evt: MouseEvent) => setCurrentMousePosition({
        x: evt.x,
        y: evt.y
    });
