...
import {children, createEffect, createSignal, For, onMount, untrack} from "solid-custom-renderer/patched-types.ts";
...
export const Menu = (props: JSX.PixieNodeProps<{visible: boolean}>) => {
    ...
    const onNavigate = controller.onKeyPress("KeyW", "KeyS", "Enter", "ShiftRight");
    const [selectIndex, setSelectIndex] = createSignal(0);
    const [menu, setMenu] = createSignal(["Menu Item One", "Menu Item Two", "Menu Item Three"]);
    ...
    createEffect(() => {
        ...
        setSelectIndex((idx) => Math.max(0, Math.min(untrack(menu).length-1, idx)));
    })
