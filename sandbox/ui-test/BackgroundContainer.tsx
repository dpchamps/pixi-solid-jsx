...
const FixedBackgroundContainer = (props: JSX.PixieNodeProps<FixedBackgroundContainerProps>) => {
...
const DynamicBackgroundContainer = (props: JSX.PixieNodeProps<DynamicBackgroundContainerProps>) => {
    const [containerRef, setContainerRef] = createSignal<ContainerNode>();
    ...
    createEffect(() => {
        ...
        background.height = Math.max(height, props.minHeight||0)+(props.padding||0)*2;
        background.width = Math.max(width, props.minWidth||0)+(props.padding||0)*2;
    ...
    return (
        <container x={scroll.x} y={scroll.y} ref={setContainerRef} sortableChildren={true}>
            <container x={props.padding||0} y={props.padding||0}>
                {props.children}
            </container>
        </container>
...
type PropsWithoutBackground<T extends BackgroundContainerProps = BackgroundContainerProps> =
    T extends any ? Omit<T, 'background'> : never

export const Box = (props: JSX.PixieNodeProps<BoxProps & PropsWithoutBackground>) => {
        ...
        setInnerMask(
            props.type === 'fixed' ?
                createRect({
                    ...
                    width: (props.width ||0)+(props.padding||0)-(props.margin||0)*2,
                    height: (props.height ||0)+(props.padding||0)-(props.margin||0)*2,
                    fill: "white"
                })
                : null
        )
    ...
    const Inner = () => <DynamicBackgroundContainer
        onScroll={props.onScroll}
        padding={Math.max(props.padding || 0, 0)}
        observe={props.observe}
        background={background}
        onUpdate={setChildUpdate}
        minHeight={Math.max(props.type === 'fixed' ? props.height-(props.padding||0)-(props.margin||0)*2 : 0, 0)}
        minWidth={Math.max(props.type === 'fixed' ? props.width-(props.padding||0)-(props.margin||0)*2 : 0, 0)}
    ...
    return (
            <container mask={mask()}>
                <BackgroundContainer
                    ...
                    background={border}
                    observe={childUpdate}
                    height={Math.max(props.type === 'fixed' ? props.height+(props.padding||0) : 0, 0)}
                    width={Math.max(props.type === 'fixed' ? props.width+(props.padding||0) : 0, 0)}
                    noScroll={true}
                >
                    {
                        innerMask()
