...
const Menu = (props: PixiNodeProps<{setEasingFn: (easingFn: EasingFunction) => void}>) => {
    ...
    return <render-layer zIndex={100}>
            ...
            <For each={easingFns}>
                {(easingFunction, index) => {
                    return <text
                        ...
                        style={{fontWeight: currentSelection() === index() ? "bold" : "normal"}}
                    >
                        {easingFunction.name}
                    </text>
                }}
...
export const BasicCoroutineExample = () => {
    ...
    return (
        <>
            ...
            <CoroutineContainer from={0} to={Math.max(windowDimensions().innerWidth, windowDimensions().innerHeight)} duration={2_000} replay={true} delay={500} easingFn={easingFn()}>
                {(currentStep) => {
                    return <sprite
