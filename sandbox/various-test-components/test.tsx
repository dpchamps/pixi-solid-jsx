...
const Child = () => {
    ...
    return <sprite x={100} y={100} width={width()} texture={assetStore?.assetCache.textures?.['fire']}/>
}
export const TestComponent = () => {
    ...
    return (
        <Application background={'#ecdddd'} width={500} height={500} eventMode={'static'}>
            <AssetsProvider assets={[{url: "fire.png", id: "fire"}]}>
                <container>
                    {
                        count() % 2 === 0
                            ? <text eventMode={'static'} style={{fontSize: fontSize()}} onclick={onClick}>I'm Even</text>
                            : <text eventMode={'static'} style={{fontSize: fontSize()}} onclick={onClick}>I'm Odd</text>
                    }
                    <Child/>
